...
## 前端开发注意事项：并发控制与队列机制 (Safety)
由于 open_ie_service.py 在处理多请求并发时存在数据覆盖风险，前端必须严格控制请求发送策略。

### 策略一：单次批量请求

- 如果用户一次性选择了多个文件（文件 A, B, C），前端必须将它们打包在同一个 API 请求的 files 列表中发送。

- 后端会遍历 files 列表，使用 await 确保处理完 A 再处理 B，保证入库安全。

### 策略二：项目级任务队列（必须实现）

- 如果用户分多次上传（先传 A，过一会传 B），前端必须维护一个针对 project_id 的任务队列。

逻辑：
```
检查当前 project_id 是否有状态为 processing 的任务。

如果有，禁止发送新的 /api/v1/tasks 请求，或者将新请求放入等待队列。

直到前一个任务收到 completed 或 failed 状态后，才允许发送下一个请求。
```

- 理由：防止两个独立的 HTTP 请求并发修改同一项目的图谱数据。
